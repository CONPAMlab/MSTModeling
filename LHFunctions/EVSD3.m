%% EVSD for three densities: 2 d-prime, 2-threshold
%
%

function Output=EVSD3(param, Data, Input)
% Specify parameters
dp1=param(1); % d-prime (d1 ~ d2)
dp2 = param(2); % d-prime (d2 ~ d3)
dp = [0, dp1, dp1 + dp2];
s1 = param(3); % (2 + 3 | 1)
s2 = param(4); % (3 | 1 + 2) - (2 + 3 | 1)
s = [s1 s1 + s2];
Nparam = 4;
...
p_now(1,1) = r0 + (1-r0)*p_now(1,1);
p_now(1,2) = (1-r0)*p_now(1,2);
p_now(1,3) = (1-r0)*p_now(1,3);
