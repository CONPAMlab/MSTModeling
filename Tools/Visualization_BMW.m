...
%         COI_Group_PP
        COI_PMod = zeros(length(COI_Group_PP),1);
        for i = 1:length(COI_Group_PP)
            COI_PMod(i) = sum(COI.BestModel == i)/length(COI.BestModel);
        end
        ...
        hold(axes2,'on');
        % Create image
        image([1 Nmodel],1,COI_PMod','Parent',axes2,'CDataMapping','scaled');
        % Set colormap limits
        caxis([0,1]);
        if useRFXBMS==0
            % Create xlabel
        ...
        axis(axes2,'ij');
        % Set the remaining axes properties
        if isfield(config,'ModelName')
            XTickLabel2 = config.ModelName;
        else
            XTickLabel2 =num2cell(1:Nmodel);
        end
        ...
        if useRFXBMS==1
            ...
            % Set the remaining axes properties
            if isfield(config,'ModelName')
                XTickLabel4 = config.ModelName;
            else
                XTickLabel4 =num2cell(1:Nmodel);
            end
