...
for r=1:Nrepeat
    ...
    for i=1:length(Q_BMW{1}.Criteria)
        switch Q_BMW{1}.Criteria{i}
            ...
            case 'AIC'
                if family_bool==1
                    delta_AIC_Family=zeros(Nfamily,Nfamily,Nsubj);
                    AIC_weight_Family=zeros(Nfamily,Nsubj);
                    AIC_GBF_Family=ones(Nfamily,Nfamily);
                    AIC_BestFamily=zeros(Nfamily,1);
                    AIC_Family=zeros(Nsubj,Nfamily);
                end
                ...
                for subj=1:Nsubj
                    ...
                    if family_bool==1
                        for family1=1:Nfamily
                            ...
                            AIC_Family(subj,family1)=-2*log(mean(exp(-(AIC_models(subj,fam_ind)-max(AIC_models(subj,fam_ind)))/2)))+max(AIC_models(subj,fam_ind));
                            for family2=1:Nfamily
                                delta_AIC_Family(family1,family2,:)=AIC_Family(:,family2)-AIC_Family(:,family1);
                            end
                            delta_AIC_benchmark_fam=delta_AIC_Family(family1,:,subj)-min(delta_AIC_Family(family1,:,subj));
                            AIC_weight_Family(family1,subj)=exp(-delta_AIC_benchmark_fam(family1)/2)/sum(exp(-delta_AIC_benchmark_fam/2));
                        end
                        AIC_GBF_Family=AIC_GBF_Family.*(AIC_weight_Family(:,subj)*(ones(Nfamily,1)./AIC_weight_Family(:,subj))');
                        AIC_BestFamily_0=find(AIC_Family(subj,:)==min(AIC_Family(subj,:)));
                        AIC_BestFamily(subj)=AIC_BestFamily_0(1);
                    end
                end
                MC_BMW{r}.AIC.BestModel=AIC_BestModel;
                for i = 1:Nmodel
                    pBestModel(i) = sum(AIC_BestModel == i)/Nsubj;
                end
                MC_BMW{r}.AIC.pBestModel=pBestModel;
                ...
                if family_bool==1
                    MC_BMW{r}.AIC.BestFamily=AIC_BestFamily;
                    MC_BMW{r}.AIC.AIC_GBF_Family=AIC_GBF_Family;
                    MC_BMW{r}.AIC.delta_AIC_Family=delta_AIC_Family;
                    MC_BMW{r}.AIC.AIC_weight_Family=AIC_weight_Family';
                    allcumsum_AIC_fam=cumsum(AIC_weight_Family');
                    MC_BMW{r}.AIC.Group_PP_Family=allcumsum_AIC_fam(end,:)/sum(allcumsum_AIC_fam(end,:));
                end
            ...
            case 'AICc'
                ...
                for i = 1:Nmodel
                    pBestModel(i) = sum(AICc_BestModel == i)/Nsubj;
                end
                MC_BMW{r}.AICc.pBestModel=pBestModel;
            ...
            case 'BIC'
                ...
                for i = 1:Nmodel
                    pBestModel(i) = sum(BIC_BestModel == i)/Nsubj;
                end
                MC_BMW{r}.BIC.pBestModel=pBestModel;
